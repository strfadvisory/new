...
const LeftPanel: React.FC<LeftPanelProps> = ({ isCollapsed, onToggle, selectedYearData, excelData, onFeeApply, effectiveMonthlyFee, totalHousingUnits: propTotalUnits, onHousingUnitsChange, yearPriorityConfigs: propYearPriorityConfigs = {}, onYearPriorityApply }) => {
  ...
  // Force popup refresh when configs change
  React.useEffect(() => {
    const handleConfigUpdate = () => {
      console.log('[LeftPanel] Config updated, forcing popup refresh');
      setForceUpdateKey(prev => prev + 1);
    };

    window.addEventListener('yearPriorityUpdated', handleConfigUpdate);
    
    return () => {
      window.removeEventListener('yearPriorityUpdated', handleConfigUpdate);
    };
  }, []);
  ...
  return (
    <div style={{
      ...
      />
      <YearPriorityPopup
        key={`${popupYearBeingEdited || year}-${forceUpdateKey}-${(() => {
